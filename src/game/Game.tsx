...
export default function Game(){
 ...
 useEffect(()=>{
  const key=(event:KeyboardEvent)=>{
   if(event.repeat||event.altKey||event.metaKey||event.ctrlKey||modal)return;
 ...
 <div className="cover-art"><Stage3D chapter={1} activePoint={0} reduced={reduced} onInspect={()=>{}} inspected={[]} cover/></div>
 <div className="cover-copy"><div className="eyebrow"><span/>AN INTERACTIVE 3D STORY · EPISODE 01</div><p className="cover-kicker">욥기</p><h1>대답 없는<br/><em>법정</em></h1><p className="english-title">When God Does Not Explain</p><p className="cover-description">고난의 이유를 찾는 대신,<br/>설명할 수 없는 사람 곁에 머무는 이야기.</p><button className="start-button" onClick={()=>dispatch({type:"start"})}>이야기 시작하기 <Icon kind="arrow"/></button><div className="cover-facts"><span>6개의 장</span><span>성찰 포함 약 15–20분</span><span>3D 탐색 · 24개의 시선</span></div></div>
 ...
 {modal==="about"&&<Modal title="작품과 읽기 안내" close={()=>setModal(null)}><p>욥기: 대답 없는 법정<br/><em>The Book of Job: When God Does Not Explain</em></p><p>이 작품은 욥기를 바탕으로 한 3D 탐색형 인터랙티브 노벨입니다. Blender로 제작한 여섯 장의 실제 GLB 장면을 회전하고 확대하며 24개의 장소와 사물을 관찰합니다. 관찰 뒤에 대화 선택이 열립니다. 장면은 고대 세계를 상상한 양식화된 조형이며, 고고학적 복원이나 인물의 실제 외모를 주장하지 않습니다.</p><p>가족의 죽음, 질병, 깊은 탄식이 등장합니다. 필요할 때 멈추거나 창을 닫아도 됩니다. 고통스러운 장면을 잔혹하게 묘사하지 않습니다.</p><p>6장·24패널을 읽고 각 장의 질문과 해설에 머무르면 약 15–20분을 예상합니다. 읽는 속도에 따라 달라지며 기다리도록 강제하지 않습니다.</p><p>성경 문장의 장문 복제를 피하고 한국어로 새로 요약했습니다. 욥기 28장의 지혜 시와 엘리후의 연설(32–37장) 등은 에피소드 길이상 압축했습니다. 등장인물의 외모와 ‘법정’의 시각적 구도는 문학적 상상입니다.</p><p>로그인·DB·분석 도구가 없으며 선택이나 개인 정보를 전송·저장하지 않습니다. 호스팅 제공자의 일반적인 접속 처리는 별개입니다.</p><h3>본문 및 편집 참고</h3><ul><li><a href="https://www.biblegateway.com/passage/?search=Job+1-21&version=KJV" target="_blank" rel="noreferrer">욥기 1–21장 · 성경 본문</a></li><li><a href="https://www.biblegateway.com/passage/?search=Job+22-42&version=KJV" target="_blank" rel="noreferrer">욥기 22–42장 · 성경 본문</a></li><li><a href="https://enterthebible.org/courses/job/lessons/summary-of-job/" target="_blank" rel="noreferrer">Luther Seminary · 욥기 개관</a></li></ul><p className="muted">3D: 직접 제작한 Blender 메시 466개, 장면 6개. Three.js 실시간 조명과 그림자. 장면 원본과 생성 코드는 저장소에 포함합니다. 이미지: Higgsfield 생성 일러스트 5종과 재구도. 음향: 브라우저에서 합성한 낮은 바람. 본문·해설은 전문 목회자나 성서학자의 최종 감수를 받지 않았습니다.</p></Modal>}
